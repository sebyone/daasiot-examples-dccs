...
import { FlashStatus, LoaderOptions, Resource, TarFile } from '@/types';
...
const DaaSUpdater = () => {
  ...
  const fetchFirmwareData = async (deviceName: string) => {
    try {
      ...
      const imageResource = deviceModel.resources?.find((resource: Resource) => resource.resource_type === 4)?.link;
      const firmwareResources = deviceModel.resources?.find(
        (resource: Resource) => resource.resource_type === 5 && resource.name === 'firmware'
      )?.link;
